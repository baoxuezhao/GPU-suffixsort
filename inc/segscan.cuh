...
* @{
*/
#ifndef __SUFSORT_SEGSCAN__H
#define __SUFSORT_SEGSCAN__H
...
* (returned for reuse)
*/
//group length >= 8   to 1024(blk_size = 128) / 2048(blk_size = 256)
//if group length <= 4, each thread load from multiple groups,
//otherwise, each thread load from one group
//TODO: change multiply and divide to bit op
template <class T, typename traits>
inline __device__
void loadForSegmentedScanSharedChunkFromMem4(
                                        T *s_odata,
                                        T threadScan0[4],
                                        T threadScan1[4],
                                        ...
                                        const T 		*d_idata,
                                        unsigned int	*d_group_start,
                                        unsigned int	*d_group_len,
                                        ...
                                        )
{
	...
	if(groupnum <= SCAN_CTA_SIZE << 1) // (or grouplen >= SEGSCAN_ELTS_PER_THREAD/2)
				      //  multiple thread load from one group
	{
		seg = thid/(grouplen/4);	//0
		off = thid%(grouplen/4);	//0...127

		if(!isLastBlock || bid*groupnum+seg < numGroups)
		{
			start 	= d_group_start[bid*groupnum+seg];
			len 	= d_group_len[bid*groupnum+seg];

			tempFlag.x = (off*4+0 < len) && ((tmp = d_idata[start + off*4+0])?1:0);
			tempData.x = (off*4+0 < len)? tmp:op.identity();

			tempFlag.y = (off*4+1 < len) && ((tmp = d_idata[start + off*4+1])?1:0);
			tempData.y = (off*4+1 < len)? tmp:op.identity();

			tempFlag.z = (off*4+2 < len) && ((tmp = d_idata[start + off*4+2])?1:0);
			tempData.z = (off*4+2 < len)? tmp:op.identity();

			tempFlag.w = (off*4+3 < len) && ((tmp = d_idata[start + off*4+3])?1:0);
			tempData.w = (off*4+3 < len)? tmp:op.identity();
		...
		{
			tempFlag.x = tempFlag.y = tempFlag.z = tempFlag.w = 0;
			tempData.x = tempData.y = tempData.z = tempData.w = op.identity();
		}
        ...
        // UINT_MAX
        indexVec[0] =
            (start + off*4 + 1 + 0) * tempFlag.x + (1 - tempFlag.x) * UINT_MAX;
        indexVec[1] =
            (start + off*4 + 1 + 1) * tempFlag.y + (1 - tempFlag.y) * UINT_MAX;
        indexVec[2] =
            (start + off*4 + 1 + 2) * tempFlag.z + (1 - tempFlag.z) * UINT_MAX;
        indexVec[3] =
            (start + off*4 + 1 + 3) * tempFlag.w + (1 - tempFlag.w) * UINT_MAX;
	}
	else	//grouplen == 2
	{
		seg = thid*(4/grouplen);
		...
		if(!isLastBlock || bid*groupnum+seg < numGroups)
		{
			...
			tempFlag.x = (tmp = d_idata[start])?1:0;
			tempData.x = tmp;

			tempFlag.y = (tmp = d_idata[start+1])?1:0;
			tempData.y = tmp;
		...
		{
			tempFlag.x = tempFlag.y = 0;
			tempData.x = tempData.y = op.identity();
		}

		if(!isLastBlock || bid*groupnum+seg+1< numGroups)
		{
			start 	= d_group_start[bid*groupnum+seg+1];
			len 	= d_group_len[bid*groupnum+seg+1];

			tempFlag.z = (tmp = d_idata[start])?1:0;
			tempData.z = tmp;

			tempFlag.w = (tmp = d_idata[start+1])?1:0;
			tempData.w = tmp;
		}
		else
		{
			tempFlag.z = tempFlag.w = 0;
			tempData.w = tempData.w = op.identity();
		}
	}
    ...
    if (traits::isBackward())
    {
        ...
        m_index = max(max(max(indexVec[0], indexVec[1]), indexVec[2]), indexVec[3]);
    }
    else
    {
        // Compute minimum of 4 indices
        m_index = min(min(min(indexVec[0], indexVec[1]), indexVec[2]), indexVec[3]);
    }
	...
	if(groupnum <= SCAN_CTA_SIZE << 1) // (or grouplen >= SEGSCAN_ELTS_PER_THREAD)
								  //  multiple thread load from one group
	{
		if(!isLastBlock || bid*groupnum+seg+groupnum/2< numGroups)
		{
			start 	= d_group_start[bid*groupnum+seg+groupnum/2];
			len 	= d_group_len[bid*groupnum+seg+groupnum/2];
			...
			{
				tempFlag.x = (off*4+1024+0 < len) && ((tmp = d_idata[start + off*4+1024+0])?1:0);
	        		tempData.x = (off*4+1024+0 < len)? tmp:op.identity();

				tempFlag.y = (off*4+1024+1 < len) && ((tmp = d_idata[start + off*4+1024+1])?1:0);
	        		tempData.y = (off*4+1024+1 < len)? tmp:op.identity();

				tempFlag.z = (off*4+1024+2 < len) && ((tmp = d_idata[start + off*4+1024+2])?1:0);
	        		tempData.z = (off*4+1024+2 < len)? tmp:op.identity();

				tempFlag.w = (off*4+1024+3 < len) && ((tmp = d_idata[start + off*4+1024+3])?1:0);
	        		tempData.w = (off*4+1024+3 < len)? tmp:op.identity();
			}
		...
		if(grouplen < 2048)
		{
			indexVec[0] = (start + off*4 + 1 + 0) * tempFlag.x + (1 - tempFlag.x) * UINT_MAX;
			indexVec[1] = (start + off*4 + 1 + 1) * tempFlag.y + (1 - tempFlag.y) * UINT_MAX;
			indexVec[2] = (start + off*4 + 1 + 2) * tempFlag.z + (1 - tempFlag.z) * UINT_MAX;
			indexVec[3] = (start + off*4 + 1 + 3) * tempFlag.w + (1 - tempFlag.w) * UINT_MAX;
		}
		else
		{
			indexVec[0] = (start + off*4 + 1024 + 1 + 0) * tempFlag.x + (1 - tempFlag.x) * UINT_MAX;
			indexVec[1] = (start + off*4 + 1024 + 1 + 1) * tempFlag.y + (1 - tempFlag.y) * UINT_MAX;
			indexVec[2] = (start + off*4 + 1024 + 1 + 2) * tempFlag.z + (1 - tempFlag.z) * UINT_MAX;
			indexVec[3] = (start + off*4 + 1024 + 1 + 3) * tempFlag.w + (1 - tempFlag.w) * UINT_MAX;
		}

	}
	else
	{
		seg = thid*(4/grouplen);	 					 //0
		//off = thid%(grouplen/SEGSCAN_ELTS_PER_THREAD); //0...127
		...
		{
			tempFlag.x = tempFlag.y = 1;
			tempData.x = tempData.y = op.identity();
		}

		if(!isLastBlock || bid*groupnum+seg+groupnum/2+1 < numGroups)
		{
			start 	= d_group_start[bid*groupnum+seg+groupnum/2+1];
			len 	= d_group_len[bid*groupnum+seg+groupnum/2+1];
		...
		{
			tempFlag.z = tempFlag.w = 1;
			tempData.z = tempData.w = op.identity();
		}
	}
                                            ...
                                            )
{
	...
	if(groupnum <= SCAN_CTA_SIZE << 1) // (or grouplen >= SEGSCAN_ELTS_PER_THREAD)
								  //  multiple thread load from one group
	{
		seg = threadIdx.x/(grouplen/4);	//0
		off = threadIdx.x%(grouplen/4);	//0...127

		if(!isLastBlock || blockIdx.x*groupnum+seg < numGroups)
		{
			start 	= d_group_start[blockIdx.x*groupnum+seg];
			len 	= d_group_len[blockIdx.x*groupnum+seg];

			if(off*4 < len)
				d_odata[start + off*4] = tempData.x;

			if(off*4+1 < len)
				d_odata[start + off*4+1] = tempData.y;

			if(off*4+2 < len)
				d_odata[start + off*4+2] = tempData.z;

			if(off*4+3 < len)
				d_odata[start + off*4+3] = tempData.w;
		}
		else
		{
	...
	else
	{
		...
		if(!isLastBlock || blockIdx.x*groupnum+seg < numGroups)
		{
			...
			d_odata[start]   = tempData.x;
			d_odata[start+1] = tempData.y;
		}
		else
		{
		}

		if(!isLastBlock || blockIdx.x*groupnum+seg+1< numGroups)
		{
			start 	= d_group_start[blockIdx.x*groupnum+seg+1];
			len 	= d_group_len[blockIdx.x*groupnum+seg+1];

			d_odata[start]   = tempData.z;
			d_odata[start+1] = tempData.w;
	...
	if(groupnum <= SCAN_CTA_SIZE << 1) // (or grouplen >= SEGSCAN_ELTS_PER_THREAD)
					   //  multiple thread load from one group
	{
		if(!isLastBlock || blockIdx.x*groupnum+seg+groupnum/2< numGroups)
		{
			start 	= d_group_start[blockIdx.x*groupnum+seg+groupnum/2];
			len 	= d_group_len[blockIdx.x*groupnum+seg+groupnum/2];


			if(grouplen < 2048)
			{
				if(off*4+0 < len)
					d_odata[start + off*4+0] = tempData.x;
			...
			{
				if(off*4 + 1024 + 0 < len)
					d_odata[start + off*4+1024 +0] = tempData.x;

				if(off*4 + 1024 + 1 < len)
					d_odata[start + off*4+1024 +1] = tempData.y;

				if(off*4 + 1024 + 2 < len)
					d_odata[start + off*4+1024 +2] = tempData.z;

				if(off*4 + 1024 + 3 < len)
					d_odata[start + off*4+1024 +3] = tempData.w;
			}
	...
	else
	{
		...
		if(!isLastBlock || blockIdx.x*groupnum+seg+groupnum/2+1< numGroups)
		{
			start 	= d_group_start[blockIdx.x*groupnum+seg+groupnum/2+1];
			len 	= d_group_len[blockIdx.x*groupnum+seg+groupnum/2+1];
...
template <class T, class traits>
__global__
void segmentedScan4_gt2n(T          	*d_odata,
                    const T         	*d_idata,
                    unsigned int	*d_group_start,
                    unsigned int	*d_group_len,
                    unsigned int       	numGroups,
        	    unsigned int 	groudid,
        	    unsigned int 	grouplen)
{
    SharedMemory<T> smem;
    ...
    loadForSegmentedScanSharedChunkFromMem4<T, traits>(
        ...
        flags, indices, d_idata, d_group_start, d_group_len,
        groudid, grouplen, numGroups,
        ai, bi);
    ...
    storeForSegmentedScanSharedChunkToMem4<T, traits>(
        ...
        temp, d_group_start, d_group_len,
        groudid, grouplen,
        numGroups, ai, bi);
