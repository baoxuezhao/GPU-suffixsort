...
__device__ uint getbucket(uint key)
{
	...
	uint bk = (uint)__log2f(key-1)+!(key==1);
                   ...
                   uint numElements)
{
    ...
    /*
	uint *h_counters = (uint*)malloc(16*numBlocks2*sizeof(uint));
	uint *h_countersSum = (uint*)malloc(16*numBlocks2*sizeof(uint));

	cudaMemcpy(h_counters,    counters, 16*numBlocks2*sizeof(uint), cudaMemcpyDeviceToHost);
	cudaMemcpy(h_countersSum,  countersSum, 16*numBlocks2*sizeof(uint), cudaMemcpyDeviceToHost);

	for(int i=0; i<16*numBlocks2; i++)
	{
...
__global__ void loggg(uint *d_data, uint *d_result, uint size)
{
	int tid = TID;
	if(tid >= size)
		return;
...
int main(int argc, char *argv[])
{
	...
	uint numElements = atoi(argv[1]);

	uint *h_keys = new uint[numElements];
	uint *h_vals = new uint[numElements];

	srand(10000);
	for(int i=0; i<numElements; i++)
	{
		h_keys[i] = rand()%atoi(argv[2])+1;
		h_vals[i] = i;
	}


	uint *d_keys, *d_vals;
	cudaMalloc((void**)&d_keys, numElements*sizeof(uint));
	cudaMalloc((void**)&d_vals, numElements*sizeof(uint));
	cudaMemcpy(d_keys, h_keys, numElements*sizeof(uint), cudaMemcpyHostToDevice);
	cudaMemcpy(d_vals, h_vals, numElements*sizeof(uint), cudaMemcpyHostToDevice);
	...
	RadixSort sortcls(numElements, false);
	...
	sortcls.sort(d_keys, d_vals, numElements, 4);

	//thrust::device_ptr<uint> d_key_ptr = thrust::device_pointer_cast(d_keys);
	//thrust::device_ptr<uint> d_value_ptr = thrust::device_pointer_cast(d_vals);
	//thrust::sort_by_key(d_key_ptr, d_key_ptr+numElements, d_value_ptr);

   	cudaEventRecord(stop, 0);
    	cudaEventSynchronize(stop);
    	cudaEventElapsedTime(&time, start, stop);
	printf("total sort time is %f\n", time);
	cudaEventDestroy(start);
	cudaEventDestroy(stop);


	cudaMemcpy(h_keys, d_keys, numElements*sizeof(uint), cudaMemcpyDeviceToHost);

	loggg<<<(numElements-1)/256+1, 256>>>(d_keys, d_vals, numElements);
	cudaMemcpy(h_vals, d_vals, numElements*sizeof(uint), cudaMemcpyDeviceToHost);
