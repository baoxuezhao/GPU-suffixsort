...
//	srand(time(NULL));
	...
	{
		h_par[num_par].start = i;
		v = rand()%NUM_ELEMENT_SB+1;
		if (v < 256)
			v += 256;
		h_par[num_par++].end = i+v;
	//	h_par[num_par++].end = i + NUM_ELEMENT_SB;
	}
	...
	if (!key_only)
		...
		for (i = 0; i < key_range; i += 5)
			single_block_radixsort_key_only<<<num_block, num_thread>>>(d_keys, d_par, num_interval, i, num_par);
	...
	printf("gpu sort time: %.2f ms\n", GetElapsedTime(0)*1000);
	printf("gpu sort throughput: %.2f M/s\n", num_par*NUM_ELEMENT_SB/GetElapsedTime(0)/1e6);
