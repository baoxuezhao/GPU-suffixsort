...
//__constant__ __device__ uint round_pow2[258];
...
/* the following kernel has bugs*/
__global__
void bitonic_sort_kernel_keyonly(uint *keys_global, uint32 *block_start, uint32 *block_len, uint32 num_interval, uint32 num_par)
{
	...
	__shared__ volatile uint shared_key[256];
	uint tid = threadIdx.x;
	for (uint i = interval_start; i < interval_end; i++)
	{
		uint start = block_start[i];
		uint end = start + block_len[i];
//		uint32 round2_len = round_pow2[end-start];
		...
		if (tid+start < end)
			shared_key[tid] = keys_global[tid+start];

		if (len+threadIdx.x < round2_len)	
			shared_key[len+threadIdx.x] = 0xffffffff;	
		__syncthreads(); 
   		...
   		for (uint k = 2; k <= round2_len; k *= 2)
    		{
				...
				if (tid < 1024)
				{
					...
					if (ixj > tid)
					{
						if ((tid & k) == 0)
						{
							if (shared_key[ixj] < shared_key[tid])
								_swap(shared_key[tid], shared_key[ixj]);
						}
						else
						{
							if (shared_key[tid] < shared_key[ixj])
								_swap(shared_key[tid], shared_key[ixj]);
						}
					}
         		   	}
		...
		if (start+tid< end)
			keys_global[start+tid] = shared_key[tid];
		__syncthreads();
	}
}

__global__
void bitonic_sort_kernel(uint *keys_global, uint *values_global, uint32 *block_start, uint32 *block_len, uint32 num_interval, uint32 num_par)
{
	uint interval_start = blockIdx.x*num_interval;
	uint interval_end = interval_start + num_interval;
	if (interval_end > num_par)interval_end = num_par;
	...
	__shared__ volatile uint shared_key[256];
	__shared__ volatile uint shared_value[256];
	uint tid = threadIdx.x;
	for (uint i = interval_start; i < interval_end; i++)
	{
		uint start = block_start[i];
	...
	//	uint32 round2_len = round_pow2[end-start];
		...
		{	
			shared_key[tid] = keys_global[tid+start];
			shared_value[tid] = values_global[tid+start];
		}
		if (len+threadIdx.x < round2_len)	
			shared_key[len+threadIdx.x] = 0xffffffff;	
		__syncthreads(); 
				...
				if (tid < round2_len)
				{
					...
					if (ixj > tid)
					{
						if ((tid & k) == 0)
						{
							...
							{	
								_swap(shared_key[tid], shared_key[ixj]);
								_swap(shared_value[tid], shared_value[ixj]);
							}
						}
						else
						{
		...
		{	
			keys_global[start+tid] = shared_key[tid];
			values_global[start+tid] = shared_value[tid];
		}
		__syncthreads();
	}
}
 ...
 */
 __global__ 
void single_block_radixsort(uint* keys_global, uint* values_global, uint* block_start, uint* block_len, uint num_interval, uint bit, uint num_par) {
...
//	const int ScanSize = NUM_WARPS * ScanStride;
	...
	uint keys_last[NUM_PER_THREAD];

	for (uint p = interval_start; p < interval_end; p++)
	{	
		
		uint start = block_start[p];
		uint end = start + block_len[p];
		...
		else if (threadStart < end)
		{
			...
			{
				keys_last[j]  = threadKeyData[j];
				uint bucket = bfe(keys_last[j], bit, NUM_BITS);
				counters[bucket * NUM_THREADS]++;
			}
		}
 ...
 __global__ 
void multiblock_radixsort_pass1(uint* keys_global, uint* counts_global, uint *d_block_start, uint *d_block_len, uint bit, uint block_count) {
	...
	for (uint p = block; p < block_count; p += NUM_BLOCK)
	{
		...
		for (int l = 0; l < num_element; l++)
		{
			...
			uint *write_pos = counts_global + (p*32+l)*NUM_CHANNELS;
			uint threadStart = tid + start;
			const uint* threadData = keys_global + threadStart;
			if (end > block_start+block_len)end = block_start+block_len;
			if (end < threadStart)
				valPerThread = 0;	
			else
				valPerThread = ((end-threadStart)>>8) + ((end-threadStart)&(NUM_THREADS-1)?1:0);
...
//	volatile uint* global_counters = shared1 + 512;
 ...
 __global__ 
void single_block_radixsort_key_only(uint* keys_global, uint* block_start, uint* block_len, uint num_interval, uint bit, uint num_par) {
...
__global__
void multiblock_radixsort_pass3_key_only(uint *counts_global, uint *keys_global, uint *d_block_start, 
				uint *d_block_len, uint *d_tmp_store, uint bit, uint block_count)
{
...
__global__
void single_block_radixsort1(uint* keys_global, uint* values_global, uint* block_start, uint* block_len, uint bit, uint num_seg)
{

	uint tid = threadIdx.x;
	uint lane = (WARP_SIZE - 1) & tid;
	uint warp = tid / WARP_SIZE;
	uint bid = gridDim.y * blockIdx.x + blockIdx.y;

	//uint interval_start = block * num_interval;
	//uint interval_end = interval_start + num_interval;

	if (bid >= num_seg) return;
...
//	const int ScanSize = NUM_WARPS * ScanStride;
	...
	uint start = block_start[bid];
	uint end = start + block_len[bid];
	uint valPerThread = NUM_PER_THREAD;
	uint threadStart = start + tid * valPerThread;
	uint* key_start = keys_global + start;
	uint* value_start = values_global + start;
